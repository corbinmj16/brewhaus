...
<template>
  <section>
    <h1 class="text-3xl font-bold">Find a brewery</h1>
    ...
    <div class="flex justify-between mt-4">
      <input
        type="text"
        placeholder="Search..."
        class="border border-zinc-100 rounded-full p-2 w-full"
      />
      <button class="bg-amber-500 text-white rounded-full px-4 py-2 ml-4">Search</button>
    </div>

    <div class="mt-4">
      <div v-if="isLoading"><LoaderCircle class="animate-spin text-amber-500" /></div>
      <div v-else-if="!breweries">No breweries found.</div>
      <div v-else v-for="brewery in breweries" :key="brewery.id">
      ...
      <div class="flex justify-center gap-4">
        <button
          ...
          class="mt-4 bg-amber-500 hover:bg-amber-700 hover:cursor-pointer text-white rounded-full px-4 py-2"
        >
